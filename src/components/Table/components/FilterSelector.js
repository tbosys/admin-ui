...
import useFetch from "business/hooks/useQuery";
...
export default function FilterModal(props) {
  ...
  function onSelectView(e) {
    ...
    var newFilters = filters.map(filter => {
      return { key: filter[0].split(".").reverse()[0], operation: filter[1], value: filter[2] };
    });

    props.onFilter({ filters: newFilters, columns: columns, filterName: filter.name });
  }

  return (
    <div className={classes.root}>
      <Button color="inherit" aria-haspopup="true" onClick={handleFilterIconClick}>
