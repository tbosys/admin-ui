...
import Ajax from "business/hooks/helpers/ajax";

export default function useProcessQuery({ path, limit = 25, statusField = "status" }) {
  ...
  async function fetchMore() {
    if (!pageInfo) return setError(new Error("Page Info is required to fetchMore"));
    setLoading(true);
    setData([]);
  ...
  async function fetch(filter = {}) {
    ...
    try {
      const response = await ajax.post({
        path,
        body: { ...body, filters: [...body.filters, [statusField, "=", filter.status || status]] }
      });
      setPageInfo(response.pageInfo);
  ...
  return { loading, fetch, setStatus, status, data, error, fetchMore, fetchLess, totalCount, filter };
}
