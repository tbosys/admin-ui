    ...
    <Fragment>
      <div className={classes.sectionDesktop}>
        ...
        <FilterSelector
          ...
          type="list"
          data={props.lists.filter(list => list.type == "list")}
        ...
        <FilterSelector
          ...
          type="report"
          data={props.lists.filter(list => list.type == "report")}
          table={props.schema.key}
          onFilter={props.onReport}
        ...
        <FilterSelector
          icon={TimelineIcon}
          type={"pivot"}
          data={props.lists.filter(list => list.type == "pivot")}
          table={props.schema.key}
          onFilter={props.onReport}
          onSave={props.onSavePivot}
        />

        <IconButton
          onClick={props.onFetchLast}
          aria-label="show 4 new mails"
          color="inherit"
        >
          <Badge
            style={{ zIndex: 8000 }}
            badgeContent={props.groupCount}
            color="secondary"
          >
            <ChevronLeft />
          </Badge>
        ...
        {props.selectedRows.length == 1 ? null : null}
      </div>
      ...
      {React.Children.map(props.children, (child, index) => {
        return React.cloneElement(child, {
          onClick: () =>
            child.props.onClick(child.props.name, props.selectedRows)
        });
      })}
    </Fragment>
  );
