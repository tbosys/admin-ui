...
import Table from "@tbos/components/Table";

import useProcessQuery from "@tbos/business/hooks/useProcessQuery";
            ...
            <Paper className={classes.ert}>
              ...
              >
                {props.statusList.map(status => (
                  <Tab
                    value={status}
                    label={`${status} (${countPerState[status] || 0})`}
                  />
                ))}
              </Tabs>
            </Paper>
